...
//
// Important: gradient/filter IDs must be unique per instance. When two SVGs
// on the same page declare <linearGradient id="x">, the browser only resolves
// `fill="url(#x)"` against the first one — every other instance renders black.
// React's useId() gives us a stable, SSR-safe unique suffix.
...
export function Logo({ animated = false, className, ...rest }: Props) {
  ...
  const gBubble = `bubble-${uid}`;
  const gBlob = `blob-${uid}`;
  const gBlob2 = `blob2-${uid}`;
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={gBubble} x1="100%" y1="0%" x2="0%" y2="100%">
        ...
        <linearGradient id={gBlob} x1="0%" y1="100%" x2="100%" y2="0%">
          <stop offset="0%" stopColor="#ffd9b8" stopOpacity="0.95" />
          <stop offset="100%" stopColor="#ff8a3a" stopOpacity="0" />
        </linearGradient>
        <linearGradient id={gBlob2} x1="0%" y1="0%" x2="100%" y2="100%">
